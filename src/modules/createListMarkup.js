const createListMarkup = ({ index, description, complete }) => `
    <li class="${complete ? 'completed' : ''}">
      <div>
        <input class="checkbox" type="checkbox" name="tasks" ${complete ? 'checked' : ''
} id="${index}" />
        <span ${!complete ? 'contenteditable' : ''}>${description}</span>
      </div>
